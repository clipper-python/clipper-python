...
class BundleBuilder:
    ...
    def __init__(self, logger=None, bundle_path=None):
        import os, os.path
        if logger is None:
            import logging
    ...
    @distlib_hack
    def make_wheel(self, test=True, parallel=False, debug=False):
    ...
    def make_install(self, session, test=True, debug=False, user=None):
        ...
        # TODO: add generic python install


    @distlib_hack
    def make_clean(self):
        #TODO: Doesn't properly clean up after itself yet (CLibrary 
        #object files and libraries are built in the src directory, and
        #not removed)
        import os
        import fnmatch
...
class _CLibrary(_CompiledCode):
    ...
    def compile(self, logger, dependencies, debug=False, parallel=True):
...
if __name__ == "__main__":
    import sys
    import argparse
    parser = argparse.ArgumentParser(description='Control build options')
    parser.add_argument("command", action="store", help="Build type")
    parser.add_argument("-p", "--parallel", dest="parallel", action="store_true", default=False)
    args = parser.parse_args()
    bb = BundleBuilder()
    if args.command == "wheel":
        bb.make_wheel(parallel=args.parallel)
        ...
        print("unknown command: %s" % repr(args.command))
    raise SystemExit(0)
