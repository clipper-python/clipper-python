...
%module(directors="1") clipper_python_core
...
%apply (float* INPLACE_ARRAY1, int DIM1) {(float *test_numpy_a, int test_numpy_n)};
%apply (float* INPLACE_ARRAY1, int DIM1) {(float *numpy_array, int n)}
%apply (float* INPLACE_ARRAY2, int DIM1, int DIM2) {(float *numpy_array, int n1, int n2)}
%apply (float* INPLACE_ARRAY3, int DIM1, int DIM2, int DIM3 ) {(float *numpy_array, int n1, int n2, int n3 )};
%apply (float* INPLACE_ARRAY3, int DIM1, int DIM2, int DIM3 ) {(float *numpy_array, int nu, int nv, int nw )};
%apply (float* INPLACE_ARRAY3, int DIM1, int DIM2, int DIM3 ) {(float *numpy_array, int nx, int ny, int nz )};

%apply (float* IN_ARRAY1, int DIM1) {(float *numpy_1d_in, int n)};
%apply (float* IN_ARRAY2, int DIM1, int DIM2) {(float *numpy_2d_in, int n1, int n2)};
%apply (float* IN_ARRAY3, int DIM1, int DIM2, int DIM3) {(float *numpy_3d_in, int n1, int n2, int n3)};
%apply (float* IN_ARRAY3, int DIM1, int DIM2, int DIM3) {(float *numpy_3d_in, int nu, int nv, int nw)};

%apply (float* ARGOUT_ARRAY1, int DIM1) {(float *numpy_float_1d_out, int num)}
%apply (float ARGOUT_ARRAY1[ANY]) {(float numpy_float_out[ANY])};
%apply (float ARGOUT_ARRAY2[ANY][ANY]) {(float numpy_float_out[ANY][ANY])};
%apply (float ARGOUT_ARRAY3[ANY][ANY][ANY]) {(float numpy_float_out[ANY][ANY][ANY])};

// (u,v,w) offset in fractional coordinates (e.g. for output of a unit cell)
%apply (float IN_ARRAY1[ANY]) {(float frac_offset[3])};
...
namespace clipper
{
    /* Equivalent functionality to C++11 std::to_string(), but we
     * can't use C++11 while maintaining PyPI compatibility.
     */
...
%typemap(in) const clipper::String& (clipper::String temp, clipper::String *s) {
  if (PyString_Check($input)) {
    temp = clipper::String(PyString_AsString($input),
                           PyString_Size($input));
    $1 = &temp;
